...
command! -nargs=0 -bar TrinityGotoEditWindow 
    \ call <SID>Trinity_Goto_Edit_Window()

command! -nargs=0 -bar TrinityGotoTagListWindow 
    \ call <SID>Trinity_Goto_Taglist_Window()

command! -nargs=0 -bar TrinityGotoExplorerWindow 
    \ call <SID>Trinity_Goto_NERDtree_Window()

command! -nargs=0 -bar TrinityExit
    \ call <SID>Trinity_Exit()

command! -nargs=0 -bar TrinityOpen
    \ call <SID>Trinity_Open()
...
let s:mini_src_switch        = 0
let s:mini_src_title         = "-MiniBufExplorer-"
" }}}
...
function! <SID>Trinity_GetWindowByName(winname)
    let l:i = 1
    while 1
        if l:i > winnr("$")
            return -1
        endif
        if bufname(winbufnr(l:i)) == a:winname
            return l:i
        endif
        let l:i += 1
    endwhile
...
function! <SID>Trinity_Goto_NERDtree_Window()
    let l:winno = <SID>Trinity_GetWindowByName("NERD_tree_1")
    if l:winno != -1
        execute l:winno . "wincmd w"
    endif
endfunction

function! <SID>Trinity_Goto_Taglist_Window()
    let l:winno = <SID>Trinity_GetWindowByName("__Tag_List__")
    if l:winno != -1
        execute l:winno . "wincmd w"
    endif
endfunction

function! <SID>Trinity_Goto_Edit_Window()
    let l:winno = <SID>Trinity_GetEditWinNR()
    if l:winno != -1
        execute l:winno . "wincmd w" 
    endif
endfunction
